...
class CrispMain {
  ...
  setSegments(List<String> segments) {
    ...
    segments.forEach((value) => appendScript('window.\$crisp.push(["set", "session:segments", [["$value"]]]);'));
  }

  setSessionData(Map<String, String> sessionData) {
    if (sessionData.isEmpty) return;

    sessionData.forEach((key, value) => appendScript('window.\$crisp.push(["set", "session:data", ["$key", "$value"]]);'));
  }
